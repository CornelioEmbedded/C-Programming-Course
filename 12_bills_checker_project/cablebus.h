...
float cablebus(float *p_sueldo, int *p_acumulador_cable)
{
    int otro_viaje;
    int costo = 5;
    ...
    if (*p_sueldo < costo)
    {
        printf("No tienes saldo suficiente para usar el cablebus");
    }else
    {
        *p_sueldo -= costo;
        *p_acumulador_cable += 1;
    ...
    return *p_sueldo, *p_acumulador_cable;
}